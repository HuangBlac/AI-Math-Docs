...
!!! note "本页来源与说明"
    本页内容正在从我自己的复习对话（与 ChatGPT/Claude，2026-05~06）中**逐主题蒸馏**而来，含 AI 生成成分，**待我本人校对**。配套精读见 [LFTP Ch4 经验风险最小化](ch4.1-4.4.3.md)。
...
- [ ] 经验风险最小化与一致偏差（见 [LFTP Ch4](ch4.1-4.4.3.md)）
...
- **McDiarmid 不等式**（有界差分函数的集中）——见 [LFTP Ch4 的完整鞅证明](ch4.1-4.4.3.md)，它正是用条件版 Hoeffding 引理逐步剥矩母函数得到的；
...
!!! note "来源"
    整理自我对一页讲义放缩过程的复盘（原题为手写讲义图，已据推导复述）。承接 [LFTP Ch4](ch4.1-4.4.3.md) 的覆盖数一节。

[LFTP Ch4](ch4.1-4.4.3.md) 用覆盖数把无限类的一致偏差控住，得到的是**慢速率** $\sqrt{\log\mathcal{N}/n}$。但若损失再满足一个 **Bernstein 条件（间隔条件）**——方差被超额风险控制——速率能提升到 $\log\mathcal{N}/n$（快一个平方）。
