...
但是Propostion 4.6和Ex4.15都依赖于证明的强凸性，没法去做

Propositon 4.7就是在这个基础上，将正则性条件转化为了更一般的$\Omega(\theta)$
...
在本章中，我们主要研究了以 \(1/\sqrt n\) 速度衰减的估计误差。当我们通过调整范数约束或正则化参数，在估计误差与近似误差之间进行平衡时，最终得到的总体收敛速率可能比 \(1/\sqrt n\) 更慢。

不过，这些结果所需要的假设也会更弱。我们将在第 7 章的核方法和第 9 章的神经网络中看到这种现象。
